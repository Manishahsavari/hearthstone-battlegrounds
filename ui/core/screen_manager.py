from __future__ import annotations

from dataclasses import dataclass, field
from typing import Dict, Optional, Protocol

import pygame


class Screen(Protocol):
    """Protocol for UI screens used by ScreenManager."""

    def handle_event(self, event: pygame.event.Event) -> None: ...

    def update(self, dt: float) -> None: ...

    def render(self, surface: pygame.Surface) -> None: ...


@dataclass
class _EmptyScreen:
    """Fallback screen used when nothing is registered yet."""

    background_color: tuple[int, int, int] = (10, 10, 20)

    def handle_event(self, event: pygame.event.Event) -> None:
        # No-op placeholder
        return

    def update(self, dt: float) -> None:
        # No-op placeholder
        return

    def render(self, surface: pygame.Surface) -> None:
        surface.fill(self.background_color)


@dataclass
class ScreenManager:
    """
    Simple screen router.

    It owns the active screen and forwards pygame events, update, and render
    calls to it. Other parts of the app can switch screens by name.
    """

    _screens: Dict[str, Screen] = field(default_factory=dict)
    _active_name: Optional[str] = None
    _fallback: Screen = field(default_factory=_EmptyScreen)

    def register(self, name: str, screen: Screen, *, activate: bool = False) -> None:
        """Register a screen instance under a unique name."""
        self._screens[name] = screen
        if activate or self._active_name is None:
            self._active_name = name

    @property
    def active(self) -> Screen:
        """Return the currently active screen (or fallback if none)."""
        if self._active_name is None:
            return self._fallback
        return self._screens.get(self._active_name, self._fallback)

    def set_active(self, name: str) -> None:
        """Switch to a previously registered screen."""
        if name not in self._screens:
            raise KeyError(f"Screen '{name}' is not registered.")
        self._active_name = name

    # pygame integration -------------------------------------------------

    def handle_event(self, event: pygame.event.Event) -> None:
        self.active.handle_event(event)

    def update(self, dt: float) -> None:
        self.active.update(dt)

    def render(self, surface: pygame.Surface) -> None:
        self.active.render(surface)

