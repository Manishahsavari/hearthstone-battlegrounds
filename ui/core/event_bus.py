...
print("EVENT_BUS_LOADED_FROM =", __file__, flush=True)
...
class EventBus:
    ...
    def post_to_server(self, event: dict):
        raise RuntimeError(f"post_to_server CALLED with: {event}")
    ...
    def drain_server(self, max_events: int | None = None) -> Iterable[dict[str, Any]]:
        ...
        count = len(self.server_queue) if max_events is None else min(
            max_events, len(self.server_queue)
        )
    ...
    def drain_ui(self, max_events: int | None = None) -> Iterator[dict[str, Any]]:
        ...
        count = len(self.ui_queue) if max_events is None else min(
            max_events, len(self.ui_queue)
        )
        for _ in range(count):
